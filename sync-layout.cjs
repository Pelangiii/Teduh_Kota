...
pages.forEach(file => {
  ...
  // 1. Revert header size to text-3xl md:text-4xl
  ...
  // 2. Revert circle star size and position to the user's manual edit
  // The user's edit: className="absolute -left-2 md:-left-4 -bottom-2 md:-bottom-4 w-6 md:w-8 z-20 pointer-events-none"
  // Let's replace any variant of circleStarImg className
  content = content.replace(
  ...
  // 3. Update the iframe wrapper to rounded-4xl
  content = content.replace(
